/**
 * AlgoForge — core/hmac_sha256.hpp
 *
 * Header-only SHA-256 + HMAC-SHA256 (hex) — no external crypto dependency.
 * Used by the Binance + Coinbase REST adapters for request signing.
 * Standard FIPS 180-4 SHA-256 and RFC 2104 HMAC; output matches
 * Python's hmac.new(key, msg, hashlib.sha256).hexdigest().
 */
#pragma once
#include <cstdint>
#include <cstring>
...
/* HMAC-SHA256(key, msg) → lowercase hex string (RFC 2104). */
inline std::string hmac_sha256_hex(const std::string& key, const std::string& msg) {
    uint8_t k[64] = {0};
    if (key.size() > 64) {
        uint8_t kh[32];
        detail::sha256_raw(reinterpret_cast<const uint8_t*>(key.data()), key.size(), kh);
        std::memcpy(k, kh, 32);
    } else {
    ...
    uint8_t ih[32];
    detail::sha256_raw(inner.data(), inner.size(), ih);

    std::vector<uint8_t> outer(opad, opad+64);
    outer.insert(outer.end(), ih, ih+32);
    uint8_t oh[32];
    detail::sha256_raw(outer.data(), outer.size(), oh);

    return detail::to_hex(oh, 32);
}

inline std::string sha256_hex(const std::string& msg) {
    uint8_t h[32];
    detail::sha256_raw(reinterpret_cast<const uint8_t*>(msg.data()), msg.size(), h);
    return detail::to_hex(h, 32);
}
