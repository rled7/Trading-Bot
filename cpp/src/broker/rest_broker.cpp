...
/* =========================================================================
 * Concrete adapter stubs — constructors only (trading methods are TODO)
 *
 * These satisfy the linker for the forward-declared classes in the header.
 * Each adapter worker will replace this file's stub with a full .cpp.
 * =========================================================================*/

OandaBroker::OandaBroker(BrokerConfig cfg) : RestBroker(std::move(cfg)) {}
AlpacaBroker::AlpacaBroker(BrokerConfig cfg) : RestBroker(std::move(cfg)) {}
IbkrBroker::IbkrBroker(BrokerConfig cfg) : RestBroker(std::move(cfg)) {}
...
/* ── Stub IBroker trading methods for concrete adapters ──
 * Each adapter worker overrides these in its own .cpp.
 * The "unimplemented" pattern mirrors Python's _todo(). */

#define STUB_RETURN_ERR  return AF_ERR_NOT_CONNECTED;
#define STUB_FILLED(f)   if (f) *f = 0; return AF_ERR_NOT_CONNECTED;

/* ── OandaBroker stubs ── */
AF_Error OandaBroker::get_account(AF_AccountInfo&)           const { STUB_RETURN_ERR }
AF_Error OandaBroker::get_tick(const char*, AF_Tick&)         const { STUB_RETURN_ERR }
AF_Error OandaBroker::get_symbol_info(const char*, AF_SymbolInfo&) const { STUB_RETURN_ERR }
AF_Error OandaBroker::get_bars(const char*, AF_Timeframe, int,
                               AF_Bar*, int* f)               const { STUB_FILLED(f) }
AF_Error OandaBroker::place_order(const char*, AF_OrderType, double, double,
                                  double, double, uint32_t, const char*,
                                  AF_Order&)                       { STUB_RETURN_ERR }
AF_Error OandaBroker::close_position(uint64_t, double)             { STUB_RETURN_ERR }
AF_Error OandaBroker::modify_position(uint64_t, double, double)    { STUB_RETURN_ERR }
AF_Error OandaBroker::cancel_order(uint64_t)                       { STUB_RETURN_ERR }
std::vector<AF_Position>   OandaBroker::get_positions()      const { return {}; }
std::vector<AF_Order>      OandaBroker::get_orders()         const { return {}; }
std::optional<AF_Position> OandaBroker::get_position(uint64_t) const { return std::nullopt; }
void OandaBroker::_on_connect()                                    {}
std::map<std::string,std::string> OandaBroker::_auth_headers() const { return {}; }
std::string OandaBroker::_to_broker_symbol(const std::string& s)   const {
    ...
    /* Insert _ after first 3 chars for 6-char symbols */
    if (result.size() == 6) result.insert(3, "_");
    return result;
}
...
std::string OandaBroker::_acct_path(const std::string& sub) const {
    return "/accounts/" + cfg_.account_id + (sub.empty() ? "" : "/" + sub);
}
AF_Position OandaBroker::_trade_to_position(const JsonVal&) { return {}; }
AF_Order    OandaBroker::_raw_order_to_order(const JsonVal&) { return {}; }

/* ── AlpacaBroker stubs ── */
AF_Error AlpacaBroker::get_account(AF_AccountInfo&)           const { STUB_RETURN_ERR }
AF_Error AlpacaBroker::get_tick(const char*, AF_Tick&)         const { STUB_RETURN_ERR }
AF_Error AlpacaBroker::get_symbol_info(const char*, AF_SymbolInfo&) const { STUB_RETURN_ERR }
AF_Error AlpacaBroker::get_bars(const char*, AF_Timeframe, int,
                                AF_Bar*, int* f)               const { STUB_FILLED(f) }
AF_Error AlpacaBroker::place_order(const char*, AF_OrderType, double, double,
                                   double, double, uint32_t, const char*,
                                   AF_Order&)                       { STUB_RETURN_ERR }
AF_Error AlpacaBroker::close_position(uint64_t, double)             { STUB_RETURN_ERR }
AF_Error AlpacaBroker::modify_position(uint64_t, double, double)    { STUB_RETURN_ERR }
AF_Error AlpacaBroker::cancel_order(uint64_t)                       { STUB_RETURN_ERR }
std::vector<AF_Position>   AlpacaBroker::get_positions()      const { return {}; }
std::vector<AF_Order>      AlpacaBroker::get_orders()         const { return {}; }
std::optional<AF_Position> AlpacaBroker::get_position(uint64_t) const { return std::nullopt; }
void AlpacaBroker::_on_connect()                                    {}
std::map<std::string,std::string> AlpacaBroker::_auth_headers() const { return {}; }
...
std::string AlpacaBroker::_resolve_key(uint64_t ticket) const {
    ...
    return (it != ticket_to_id_.end()) ? it->second : "";
}
std::string AlpacaBroker::_data_url() const {
    return DEFAULT_DATA_URL;
}
AF_Position AlpacaBroker::_parse_position(const JsonVal&, uint64_t, const std::string&) const {
    return {};
}

/* ── IbkrBroker stubs ── */
AF_Error IbkrBroker::get_account(AF_AccountInfo&)           const { STUB_RETURN_ERR }
AF_Error IbkrBroker::get_tick(const char*, AF_Tick&)         const { STUB_RETURN_ERR }
AF_Error IbkrBroker::get_symbol_info(const char*, AF_SymbolInfo&) const { STUB_RETURN_ERR }
AF_Error IbkrBroker::get_bars(const char*, AF_Timeframe, int,
                              AF_Bar*, int* f)               const { STUB_FILLED(f) }
AF_Error IbkrBroker::place_order(const char*, AF_OrderType, double, double,
                                 double, double, uint32_t, const char*,
                                 AF_Order&)                       { STUB_RETURN_ERR }
AF_Error IbkrBroker::close_position(uint64_t, double)             { STUB_RETURN_ERR }
AF_Error IbkrBroker::modify_position(uint64_t, double, double)    { STUB_RETURN_ERR }
AF_Error IbkrBroker::cancel_order(uint64_t)                       { STUB_RETURN_ERR }
std::vector<AF_Position>   IbkrBroker::get_positions()      const { return {}; }
std::vector<AF_Order>      IbkrBroker::get_orders()         const { return {}; }
std::optional<AF_Position> IbkrBroker::get_position(uint64_t) const { return std::nullopt; }
void IbkrBroker::_on_connect()                                    {}
int64_t IbkrBroker::_resolve_conid(const std::string&) const { return 0; }
AF_Position IbkrBroker::_parse_position(const JsonVal&) const { return {}; }
AF_Order    IbkrBroker::_parse_order(const JsonVal&) const { return {}; }
double IbkrBroker::_safe_float(const JsonVal& v, double def) {
    return v.is_num() ? v.num : def;
}
double IbkrBroker::_summary_amount(const JsonVal&, const std::string&) { return 0.0; }

/* ── BinanceBroker stubs ── */
AF_Error BinanceBroker::get_account(AF_AccountInfo&)           const { STUB_RETURN_ERR }
AF_Error BinanceBroker::get_tick(const char*, AF_Tick&)         const { STUB_RETURN_ERR }
AF_Error BinanceBroker::get_symbol_info(const char*, AF_SymbolInfo&) const { STUB_RETURN_ERR }
AF_Error BinanceBroker::get_bars(const char*, AF_Timeframe, int,
                                 AF_Bar*, int* f)               const { STUB_FILLED(f) }
AF_Error BinanceBroker::place_order(const char*, AF_OrderType, double, double,
                                    double, double, uint32_t, const char*,
                                    AF_Order&)                       { STUB_RETURN_ERR }
AF_Error BinanceBroker::close_position(uint64_t, double)             { STUB_RETURN_ERR }
AF_Error BinanceBroker::modify_position(uint64_t, double, double)    { STUB_RETURN_ERR }
AF_Error BinanceBroker::cancel_order(uint64_t)                       { STUB_RETURN_ERR }
std::vector<AF_Position>   BinanceBroker::get_positions()      const { return {}; }
std::vector<AF_Order>      BinanceBroker::get_orders()         const { return {}; }
std::optional<AF_Position> BinanceBroker::get_position(uint64_t) const { return std::nullopt; }
void BinanceBroker::_on_connect()                                    {}
std::map<std::string,std::string> BinanceBroker::_auth_headers() const { return {}; }
std::string BinanceBroker::_to_broker_symbol(const std::string& s) const {
    /* BTC/USD → BTCUSD (strip / and -) */
    std::string result = s;
    result.erase(std::remove(result.begin(), result.end(), '/'), result.end());
    result.erase(std::remove(result.begin(), result.end(), '-'), result.end());
    return result;
...
BinanceBroker::signed_params(std::map<std::string,std::string> params,
                             long long /*timestamp_ms*/) const {
    return params;
}
std::string BinanceBroker::hmac_sha256_hex(const std::string&, const std::string&) const {
    return "";
}

/* ── CoinbaseBroker stubs ── */
AF_Error CoinbaseBroker::get_account(AF_AccountInfo&)           const { STUB_RETURN_ERR }
AF_Error CoinbaseBroker::get_tick(const char*, AF_Tick&)         const { STUB_RETURN_ERR }
AF_Error CoinbaseBroker::get_symbol_info(const char*, AF_SymbolInfo&) const { STUB_RETURN_ERR }
AF_Error CoinbaseBroker::get_bars(const char*, AF_Timeframe, int,
                                  AF_Bar*, int* f)               const { STUB_FILLED(f) }
AF_Error CoinbaseBroker::place_order(const char*, AF_OrderType, double, double,
                                     double, double, uint32_t, const char*,
                                     AF_Order&)                       { STUB_RETURN_ERR }
AF_Error CoinbaseBroker::close_position(uint64_t, double)             { STUB_RETURN_ERR }
AF_Error CoinbaseBroker::modify_position(uint64_t, double, double)    { STUB_RETURN_ERR }
AF_Error CoinbaseBroker::cancel_order(uint64_t)                       { STUB_RETURN_ERR }
std::vector<AF_Position>   CoinbaseBroker::get_positions()      const { return {}; }
std::vector<AF_Order>      CoinbaseBroker::get_orders()         const { return {}; }
std::optional<AF_Position> CoinbaseBroker::get_position(uint64_t) const { return std::nullopt; }
void CoinbaseBroker::_on_connect()                                    {}
std::string CoinbaseBroker::_to_broker_symbol(const std::string& s) const {
    /* BTCUSD → BTC-USD, BTC/USD → BTC-USD */
    std::string result = s;
    /* Replace / with - */
    for (char& c : result) if (c == '/') c = '-';
    /* If no dash already and length 6, insert at position 3 */
    if (result.find('-') == std::string::npos && result.size() == 6) {
        result.insert(3, "-");
    }
    return result;
}
std::string CoinbaseBroker::_from_broker_symbol(const std::string& s) const {
    /* BTC-USD → BTCUSD */
    std::string result = s;
    result.erase(std::remove(result.begin(), result.end(), '-'), result.end());
    return result;
}
HttpResponse CoinbaseBroker::do_http_request(
        const std::string& method,
        const std::string& url,
        const std::string& body,
        const std::map<std::string,std::string>& hdrs) const {
    /* Coinbase requires HMAC timestamp headers injected around the call.
     * Stub: fall through to base (no-op). Real implementation goes in
     * the Coinbase adapter worker's oanda_broker.cpp / coinbase_broker.cpp. */
    return RestBroker::do_http_request(method, url, body, hdrs);
}
std::string CoinbaseBroker::sign(const std::string&, const std::string&,
