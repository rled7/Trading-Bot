/**
 * AlgoForge — tests/test_oanda_broker_main.cpp
 * Standalone runner for the OANDA broker adapter tests (Phase 1 / S5).
 * Mirrors the test_ibkr_broker_main.cpp pattern.
 */
...
void run_test(const char *name, std::function<void()> fn) {
    ...
    try {
        ...
        auto ms = std::chrono::duration<double,std::milli>(
                      std::chrono::steady_clock::now()-t0).count();
        printf("  \033[91m✗\033[0m %-70s %6.1fms\n", name, ms);
        printf("       → %s\n", e.c_str());
        g_fail++;
        g_failures.push_back({name, e});
    } catch (std::exception &ex) {
        printf("  \033[91m✗\033[0m %-70s EXCEPTION\n", name);
        printf("       → %s\n", ex.what());
        g_fail++;
...
void section(const char *title) {
    printf("\n  %s\n  %s\n", title, std::string(80,'-').c_str());
}

/* Forward declaration of the test suite */
void test_oanda_broker(RawTestFn&);

int main() {
    printf("\n  %s\n", std::string(80,'=').c_str());
    printf("  AlgoForge OANDA Broker Adapter Tests (Phase 1 / S5)\n");
    printf("  %s\n", std::string(80,'=').c_str());

    test_oanda_broker(run_test);
